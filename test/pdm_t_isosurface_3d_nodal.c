...
static void _run_test_3d_nodal(PDM_isosurface_test_utils_params_t params);
    ...
    for (int n_part_in = 0; n_part_in <= 2; n_part_in++) {
      ...
      for (int n_isovalues = 1; n_isovalues <= 3; n_isovalues += 2) {
        ...
        for (int simplex = 0; simplex <= 1; simplex++) {
          ...
          if (simplex) {
            params.elt_type = PDM_MESH_NODAL_TETRA4;
          }
          else {
...
)
{
  ...
  PDM_isosurface_test_utils_isosurface_params_dump(comm, "pdm_t_isosurface_3d_nodal", params);
