    ...
    *out_pmn = PDM_generate_mesh_nodal_from_file(comm,
    ...
    if (mesh_dimension == 2) {
      ...
      *out_pmn = PDM_generate_mesh_parallelepiped(comm,
      ...
      PDM_error("Invalid mesh_dimension %d (must be 2 or 3)", mesh_dimension);
    }
  }
}
...
)
{
  ...
  double **pvtx_coord_prev = NULL;
  PDM_malloc(pvtx_coord_prev, n_part, double *);
  for (int i_part = 0; i_part < n_part; i_part++) {
    PDM_malloc(pvtx_coord_prev[i_part], 3 * pn_vtx[i_part], double);
    memcpy(pvtx_coord_prev[i_part], pvtx_coord[i_part], sizeof(double) * 3 * pn_vtx[i_part]);
  }
  ...
  for (int i_part = 0; i_part < n_part; i_part++) {
    ...
    PDM_free(pvtx_coord_prev[i_part]);
  }
  PDM_free(pn_vtx    );
  PDM_free(pn_edge   );
  PDM_free(pvtx_coord);
