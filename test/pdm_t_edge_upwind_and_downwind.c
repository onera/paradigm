...
{
  int *upwind_face   = PDM_array_zeros_int(n_edge);
  int *downwind_face = PDM_array_zeros_int(n_edge);
  ...
  for (int iedge = 0; iedge < n_edge; iedge++) {
    ...
    for (int jvtx = 0; jvtx < 2; jvtx++) {
      ...
      for (int idx_cell = vtx_cell[ivtx]; idx_cell < vtx_cell[ivtx+1]; idx_cell++) {

        int icell = PDM_ABS(vtx_cell[idx_cell]) - 1;
        printf("icell = %d/%d\n",icell, n_cell);

        for (int idx_face = cell_face[icell]; idx_face < cell_face[icell+1]; idx_face++) {

          int iface = PDM_ABS(cell_face[idx_face]) - 1;
          printf("  idx_face = %d, iface = %d/%d\n", idx_face, iface, n_face);
          ...
          if (PDM_ABS(denom) < epsilon) {   // Epsilon is here to avoid division by 0
            ...
            if (t < 0) {
              stat = 1;
              for (int j = 0; j < 3; j++) {
                intersection[j] = vtx_coord[3*ivtx+j] + t*edge_vec[j];
              }
            }
          }
