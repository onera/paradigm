...
program testf
  ...
  if (i_rank .eq. 0) then
    write(*, *) "-- Create PDM_part_extension object (face)"
  end if
  ...
  call PDM_part_extension_compute2 (part_ext, 3)
  ...
  do i_part = 1, n_part
    ...
    !  Vertices
    call PDM_part_extension_vtx_coord_get2 (part_ext,      &
                                           0,             &
                                           i_part-1,      &
                                           n_vtx_ext,     &
                                           vtx_coord_ext, &
    ...
    call PDM_part_extension_ln_to_gn_get2 (part_ext,            &
                                          0,                   &
                                          i_part-1,            &
                                          PDM_MESH_ENTITY_VTX, &
                                          n_vtx_ext,           &
    ...
    !  Cell-face
    call PDM_part_extension_connectivity_get2 (part_ext,                        &
                                              0,                               &
                                              i_part-1,                        &
  ...
  ! Free PDM_part_extension
  call PDM_part_extension_free (part_ext)

  !  Create new PDM_part_extension object
  if (i_rank .eq. 0) then
    write(*, *) "-- Create PDM_part_extension object (vtx)"
  end if
