 ...
 )
{
  ...
  for (int i = 0; i < n_face; i++) {
    max_n_vtx = PDM_MAX (max_n_vtx, face_vtx_idx[i+1] - face_vtx_idx[i]);
  }

  int *tmp = malloc (sizeof(int) * max_n_vtx);
  for (int i = 0; i < n_face; i++) {
    int n_vtx = face_vtx_idx[i+1] - face_vtx_idx[i];

    double barycenter[3] = {0., 0., 0.};
    for (int j = face_vtx_idx[i]; j < face_vtx_idx[i+1]; j++) {
      int ivtx = face_vtx[j] - 1;
      for (int k = 0; k < 3; k++) {
        barycenter[k] += vtx_coord[3*ivtx + k];
      }
    }
    for (int k = 0; k < 3; k++) {
      barycenter[k] /= (double) n_vtx;
    }

    const double *v1 = vtx_coord + 3*(face_vtx[face_vtx_idx[i]] - 1);
    const double *v2 = vtx_coord + 3*(face_vtx[face_vtx_idx[i]+1] - 1);
    double a[2] = {v1[0] - barycenter[0],
                   v1[1] - barycenter[1]};
    double b[2] = {v2[0] - barycenter[0],
                   v2[1] - barycenter[1]};
    ...
    if (nz < 0) {
      ...
      for (int j = 0; j < n_vtx; j++) {
        int k = face_vtx_idx[i] + j;
        tmp[j] = face_vtx[k];
      }

      for (int j = 0; j < n_vtx; j++) {
        int k = face_vtx_idx[i] + j;
        face_vtx[k] = tmp[n_vtx - 1 - j];
      }
    }
  }
