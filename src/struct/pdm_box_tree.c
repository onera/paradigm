...
)
{
  ...
  for (int i = 0; i < n_pts; i++) {
    ...
    if (1 == 0) {
      printf(" fin pos_stack : %d %d %12.5e\n", i, box_id[i],  box_max_dist[i]);
    }
  }
...
)
{
  ...
  *boxes = malloc (sizeof(int) * tmp_s_boxes);
  ...
  for (int i = 0; i < n_pts; i++) {
    ...
    /*        _pt[0], _pt[1], _pt[2], upper_bound_dist2[i]); */
