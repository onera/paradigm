...
MPI_TEST_CASE("[pdm_part_to_block] - 1p - part_to_block",1) {
  ...
  SUBCASE("Merge mode") {
    ...
    PDM_free(tmp_preverse_data);

    int *preverse_stri = tmp_preverse_stri[0];
    PDM_free(tmp_preverse_stri);
...
MPI_TEST_CASE("[pdm_part_to_block] - 2p - zero global weight", 2) {
  ...
  SUBCASE("Empty parts") {
    for (int i_part = 0; i_part < n_part; i_part++) {
      pn_elmt  [i_part] = 0;
      PDM_malloc(pln_to_gn[i_part], pn_elmt[i_part], PDM_g_num_t);
      PDM_malloc(pweight  [i_part], pn_elmt[i_part], double     );
    }
  ...
  for (int i_part = 0; i_part < n_part; i_part++) {
    PDM_malloc(part_data[i_part], pn_elmt[i_part], PDM_g_num_t);
    for (int i = 0; i < pn_elmt[i_part]; i++) {
  ...
  PDM_g_num_t *block_data_expected;
  PDM_malloc(block_data_expected, dn_elmt, PDM_g_num_t);
  ...
  // Free memory
  for (int i_part = 0; i_part < n_part; i_part++) {
    PDM_free(pln_to_gn[i_part]);
    PDM_free(pweight  [i_part]);
