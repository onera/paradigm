...
)
{
  ...
  double      *extract_shared_box_center        = malloc(6 * n_extract_shared_boxes * sizeof(double     ));
  ...
  for(int i = 0; i < n_shared_boxes; ++i) {
    ...
    if(n_connect > 0) {
      ...
      for(int k = 0; k < 6; ++k) {
        extract_shared_box_center[6*n_extract_shared_boxes+k] = shared_box_center[6*i+k];
      }
