...
/**
 * \brief Compute link between a child PartMeshNodalElmts and a parent PartMeshNodalElmts
 *
 ...
 * \param [out] out_entity_to_vtx_idx     Index for ChildEntity->Vertex connectivity (if \p compute_parent_child is PDM_TRUE, size = \p n_part and for each part, size = \p out_n_entity + 1)
 * \param [out] out_entity_to_vtx         ChildEntity->Vertex connectivity (if \p compute_parent_child is PDM_TRUE, size = \p n_part)
 * \param [out] out_parent_to_entity_idx  Index for ParentEntity->ChildEntity connectivity (if \p compute_parent_child is PDM_TRUE, size = \p n_part)
 * \param [out] out_parent_to_entity      ParentEntity->ChildEntity connectivity (if \p compute_parent_child is PDM_TRUE, size = \p n_part)
 */
void
