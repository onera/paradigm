...
{
  int n_rank;
  PDM_MPI_Comm_size (mesh->comm, &n_rank);

  const int n_part = PDM_surf_mesh_n_part_get (mesh);

  *vtx_normal = (double **) malloc (sizeof(double *) * n_part);

  // Compute contributions of local faces
  for (int ipart = 0; ipart < n_part; ipart++) {
    const int n_vtx = PDM_surf_mesh_part_n_vtx_get (mesh, ipart);
    (*vtx_normal)[ipart] = (double *) malloc (sizeof(double) * n_vtx * 3);
    double *vn = (*vtx_normal)[ipart];
    for (int i = 0; i < n_vtx * 3; i++) {
      vn[i] = 0.;
    }

    const int     n_face       = PDM_surf_mesh_part_n_face_get (mesh, ipart);
    const int    *face_vtx_idx = PDM_surf_mesh_part_face_vtx_idx_get (mesh, ipart);
    const int    *face_vtx     = PDM_surf_mesh_part_face_vtx_get (mesh, ipart);
    const double *face_normal  = PDM_surf_mesh_face_normal_get (mesh, ipart);
    for (int iface = 0; iface < n_face; iface++) {
      for (int i = face_vtx_idx[iface]; i < face_vtx_idx[iface+1]; i++) {
        int ivtx = face_vtx[i] - 1;
        for (int j = 0; j < 3; j++) {
          vn[3*ivtx + j] += face_normal[3*iface + j];
        }
      }
    }
  }

  // Exchange and add contributions from distant faces
  int *send_count = (int *) malloc (sizeof(int) * n_rank);
  int *recv_count = (int *) malloc (sizeof(int) * n_rank);
  ...
  for (int ipart = 0; ipart < n_part; ipart++) {
    ...
    PDM_part_bound_t *vtx_part_bound = part->vtxPartBound;
    int n_vtx_bound = PDM_part_bound_n_elt_bound_get (vtx_part_bound);

    for (int i = 1; i <= n_vtx_bound; i++) {
      int n_connected_vtx, local_vtx;
      PDM_part_bound_bound_elt_get (vtx_part_bound,
                                    i,
                                    &local_vtx,
                                    &n_connected_vtx);

      for (int j = 0; j < n_connected_vtx; j++) {
        int jrank, jpart, procPartElt, lnum;
        PDM_part_bound_distant_elt_get (vtx_part_bound,
                                        i,
                                        j,
                                        &jrank,
                                        &jpart,
  ...
  PDM_MPI_Alltoall (send_count, 1, PDM_MPI_INT,
                    recv_count, 1, PDM_MPI_INT, mesh->comm);

  int *send_shift = (int *) malloc (sizeof(int) * (n_rank + 1));
  int *recv_shift = (int *) malloc (sizeof(int) * (n_rank + 1));
  send_shift[0] = 0;
  recv_shift[0] = 0;
  for (int i = 0; i < n_rank; i++) {
  ...
  int n_recv = recv_shift[n_rank] / 3;

  int    *send_ibuf = (int *)    malloc (sizeof(int)    * send_shift[n_rank]);
  double *send_dbuf = (double *) malloc (sizeof(double) * send_shift[n_rank]);
  for (int ipart = 0; ipart < n_part; ipart++) {
    ...
    for (int i = 1; i <= n_vtx_bound; i++) {
      ...
      PDM_part_bound_bound_elt_get (vtx_part_bound,
                                    ...
                                    &local_vtx,
                                    &n_connected_vtx);
      local_vtx = 3*(local_vtx - 1);

      for (int j = 0; j < n_connected_vtx; j++) {
        int jrank, jpart, jvtx, lnum;
        PDM_part_bound_distant_elt_get (vtx_part_bound,
                                        i,
                                        j,
                                        &jrank,
        ...
        int idx = send_shift[jrank] + send_count[jrank];
        send_ibuf[idx]     = jpart;
        send_ibuf[idx + 1] = jvtx;
        send_ibuf[idx + 2] = lnum; // unused

        send_dbuf[idx]     = vn[local_vtx];
        send_dbuf[idx + 1] = vn[local_vtx + 1];
        send_dbuf[idx + 2] = vn[local_vtx + 2];
  ...
  int *recv_ibuf = (int *) malloc (sizeof(int) * recv_shift[n_rank]);
  PDM_MPI_Alltoallv (send_ibuf, send_count, send_shift, PDM_MPI_INT,
                     recv_ibuf, recv_count, recv_shift, PDM_MPI_INT, mesh->comm);
  free (send_ibuf);

  double *recv_dbuf = (double *) malloc (sizeof(double) * recv_shift[n_rank]);
  PDM_MPI_Alltoallv (send_dbuf, send_count, send_shift, PDM_MPI_DOUBLE,
                     recv_dbuf, recv_count, recv_shift, PDM_MPI_DOUBLE, mesh->comm);
  free (send_dbuf);
  free (send_count);
  free (send_shift);
  free (recv_count);
  ...
  for (int i = 0; i < n_recv; i++) {
    int ipart = recv_ibuf[3*i];
    int ivtx  = recv_ibuf[3*i + 1] - 1;

    for (int j = 0; j < 3; j++) {
      (*vtx_normal)[ipart][3*ivtx + j] += recv_dbuf[3*i + j];
    }
  }
  free (recv_ibuf);
  free (recv_dbuf);
 ...
 )
{
  ...
  *vtx_normal = (double **) malloc (sizeof(double *) * n_part);
  ...
  double **face_normal       = (double **) malloc (sizeof(double *) * n_part);
  double **face_normal_ghost = (double **) malloc (sizeof(double *) * n_part);
  ...
  for (int ipart = 0; ipart < n_part; ipart++) {
    ...
    PDM_surf_part_t *part = mesh->part[ipart];
    const int n_face = part->n_face;
    const int n_total_face = part->nTotalFace;

    face_normal[ipart] = (double *) malloc (sizeof(double) * n_total_face);
    face_normal_ghost[ipart] = face_normal[ipart] + n_face;

    const double *_fn = PDM_surf_mesh_face_normal_get (mesh, ipart);
    memcpy (face_normal[ipart], _fn, sizeof(double) * n_face * 3);
  }

  /*
   * Initialize exchange to ghost faces
   */
  PDM_graph_bound_exch_data_init (mesh->interPartEdgeGraph,
                                  1,
                                  PDM_DOUBLE,
                                  (void **) face_normal,
                                  (void **) face_normal_ghost);

  /*
   * Finalize exchange to ghost faces
   */

  PDM_graph_bound_exch_data_wait (mesh->interPartEdgeGraph);
