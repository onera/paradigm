...
)
{
  ...
  if (dbg_enabled) {
    ...
    // double *cellA_volume;
 // PDM_malloc(cellA_volume,n_cellA,double);
    // double *cellA_center;
 // PDM_malloc(cellA_center,n_cellA * 3,double);
    // PDM_geom_elem_polyhedra_properties_triangulated(1,
    //                                                 n_cell       [0],
    //                                                 n_face       [0],
    //                                                 face_vtx_idx [0],
  ...
  if (0 == 1) {
    for (int imesh = 0; imesh < 2; imesh++) {
      if (mi->mesh_nodal[imesh] != NULL) {
        ...
        char prefix[999];
        sprintf(prefix, "mesh_intersection_extract_%d", imesh);
        PDM_part_mesh_nodal_dump_vtk(extract_pmn,
                                     geom_kind,
                                     prefix);
      }
...
)
{
  ...
  if (tetraisation_pt_type == 2) {
    // PDM_malloc(mi->tetraisation_pt_coord,3,double);
