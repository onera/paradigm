...
)
{
  ...
  double sum_w = 0.0;
  for (i = 0; i < n_vtx; i++) {
    weights[i] = (tan_half_theta[(i + n_vtx - 1)%n_vtx] + tan_half_theta[i]) * inv_dist[i];
    sum_w += weights[i];
  }
