...
{
  // printf("PDM_geom_elem_edge_upwind_and_downwind_2d \n");
  // printf("PDM_geom_elem_edge_upwind_and_downwind_2d \n");
  // printf("PDM_geom_elem_edge_upwind_and_downwind_2d \n");
  // printf("PDM_geom_elem_edge_upwind_and_downwind_2d %i / %i \n", n_face, n_edge);
  ...
  if(0 == 1) {
    PDM_log_trace_connectivity_int(face_edge_idx, face_edge, n_face, "face_edge ::");
    PDM_log_trace_connectivity_int(edge_vtx_idx , edge_vtx, n_edge, "edge_vtx ::");
  ...
  int *is_visited_edge = PDM_array_zeros_int(10*n_edge);
  int *visited_edge;
  PDM_malloc(visited_edge, 10*n_edge, int);
  int n_visited_edge = 0;
  ...
  for (int iedge = 0; iedge < n_edge; iedge++) {
    ...
    /* Reset visited faces */
    for (int i = 0; i< n_visited_edge; i++) {
      is_visited_edge[visited_edge[i]] = 0;
    }
    n_visited_edge = 0;
    ...
    for (int idx_vtx = 0; idx_vtx < 2; idx_vtx++) {
      ...
      int n_face_per_vtx = (vtx_face_idx[vtx_id+1]-vtx_face_idx[vtx_id]);
      int *order;
      PDM_malloc(order, n_face_per_vtx, int);

      if(face_ln_to_gn != NULL) {
        PDM_g_num_t *vtx_face_gnum;
        PDM_malloc(vtx_face_gnum, n_face_per_vtx, PDM_g_num_t);
        int idx_write = 0;
        for (int idx_face = vtx_face_idx[vtx_id]; idx_face < vtx_face_idx[vtx_id+1]; idx_face++) {
          int face_id = PDM_ABS(vtx_face[idx_face]) - 1;
          ...
          // printf("iedge= %i / vtx_id = %i / face_id = %i \n", iedge, vtx_id, face_id);
          vtx_face_gnum[idx_write++] = face_ln_to_gn[face_id];
        }
        PDM_order_gnum_s(vtx_face_gnum, 1, order, n_face_per_vtx);
        PDM_free(vtx_face_gnum);
      ...
      for (int idx_face0 = vtx_face_idx[vtx_id]; idx_face0 < vtx_face_idx[vtx_id+1]; idx_face0++) {
        int idx_face = order[idx_face0-vtx_face_idx[vtx_id]]+vtx_face_idx[vtx_id];
        ...
        /* Check if one face has the 2 vertex, it is an adjacent cell to the edge */
        int has_face_edge = 0;
        for (int idx_edge = face_edge_idx[face_id]; idx_edge < face_edge_idx[face_id+1]; idx_edge++) {

          int edge_id = PDM_ABS(face_edge[idx_edge]) - 1;
          for (int i = edge_vtx_idx[edge_id]; i < edge_vtx_idx[edge_id+1]; i++) {
            if (edge_vtx[i] - 1 == ivtx1) {
              for (int j = edge_vtx_idx[edge_id]; j < edge_vtx_idx[edge_id+1]; j++) {
                if ((j != i) && (edge_vtx[j] - 1 == ivtx2)) {
                  has_face_edge = 1;
                  break;
                }
              }
        ...
        for (int idx_face2 = face_edge_idx[face_id]; idx_face2 < face_edge_idx[face_id+1]; idx_face2++) {
          ...
          if (has_face_edge == 1) {
            ...
            visited_edge[n_visited_edge++] = edge_id;
          }

          if (is_visited_edge[edge_id]) {
            continue;
          }
          ...
          if(idx_vtx == 0) {
            a1 = &vtx_coord[3*ivtx2];
            a2 = &vtx_coord[3*ivtx1];
          } else {
            assert(idx_vtx == 1);
            a1 = &vtx_coord[3*ivtx1];
            a2 = &vtx_coord[3*ivtx2];
          }

          int shift = edge_vtx_idx[edge_id];

          int t_vtx1 = edge_vtx[shift  ] - 1;
          int t_vtx2 = edge_vtx[shift+1] - 1;

          b1 = &vtx_coord[3*t_vtx1];
          b2 = &vtx_coord[3*t_vtx2];
          double u = 0.;
          double v = 0.;
          ...
          if (stat == PDM_LINE_INTERSECT_YES) {
            ...
            if (idx_vtx == 1) {
              upwind_face[iedge] = face_id;
              upwind_edge[iedge] = edge_id;
              memcpy(upwind_point + 3*iedge, intersection_coord, sizeof(double) * 3);
            } else {
              downwind_face[iedge] = face_id;
              downwind_edge[iedge] = edge_id;
              memcpy(downwind_point + 3*iedge, intersection_coord, sizeof(double) * 3);
            }
            break;
          }
        }
  ...
  PDM_free(is_visited_edge);
  PDM_free(visited_edge);
