...
#define _N_VERTICES_AUTO_MAX   20 /* Size of local temporary coordinates
  ...
  if (PDM_ABS(face_normal[0]) > 1.e-12 || PDM_ABS(face_normal[1]) > 1.e-12) {
    ...
    if (n_vertices > _N_VERTICES_AUTO_MAX) {
      _tmp_coords_p  = malloc (sizeof(fvmc_coord_t) * n_vertices*3);
...
fvmc_triangulate_state_create(const int  n_vertices_max)
{
  ...
  this_state = malloc (sizeof(fvmc_triangulate_state_t));

  if (n_vertices_max > 3) {
    this_state->triangle_vertices = malloc (sizeof(int) * (n_vertices_max - 2) * 3);
    this_state->coords            = malloc (sizeof(fvmc_coord_t) * n_vertices_max*3);
                         ...
                         fvmc_triangulate_state_t  *const state)
{
  ...
  if (n_vertices > state->n_vertices_max) {
    ...
    state->coords            = realloc (state->coords,            sizeof(fvmc_coord_t) * n_vertices_max*3);
