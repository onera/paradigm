 ...
 )
{
  ...
  if (_a_eq_b(vtx_xy[1], xy[1]) &&
      _a_eq_b(vtx_xy[0], xy[0])) {
    /* Point on vertex */
    return PDM_POLYGON_INSIDE;
  }
  ...
  for (int i = 0; i < n_vtx; i++) {
    const double *pi  = vtx_xy + 2*i;
    const double *pip = vtx_xy + 2*((i+1)%n_vtx);

    if (_a_eq_b(pip[1], xy[1])) {
      if (_a_eq_b(pip[0], xy[0])) {
        /* Point on vertex */
        return PDM_POLYGON_INSIDE;
      } else {
        if (_a_eq_b(pi[1], xy[1]) &&
            ((pip[0] > xy[0]) == (pi[0] < xy[0]))) {
          /* Point on edge */
          return PDM_POLYGON_INSIDE;
        }
      }
    ...
    if (_crossing(pi[1], pip[1], xy[1])) {

      double det = (pi[0] - xy[0])*(pip[1] - xy[1]) - (pi[1] - xy[1])*(pip[0] - xy[0]);
      if (PDM_ABS(det) < eps) {
        /* Point on edge */
        return PDM_POLYGON_INSIDE;
      }
      ...
      if (pi[0] >= xy[0]) {
        if (pip[0] > xy[0]) {
          _modify_w (pi[1], pip[1], &w);
        } else {
          if (_right_crossing(det, pi[1], pip[1])) {
            _modify_w (pi[1], pip[1], &w);
          }
        }
      } else {
