...
static void
_intersect_bounding_boxes_and_redistribute
(
 PDM_ol_t       *ol,
 PDM_box_set_t **boxesA,
 PDM_box_set_t **boxesB,
 ...
 )
{
  ...
  *blockA_boxesB_idx = PDM_array_new_idx_from_sizes_int (blockA_boxesB_stride, *n_elt_blockA);
  ...
  distribA->list = (int *) malloc (sizeof(int) * distribA->index[n_rank]);
  distribB->list = (int *) malloc (sizeof(int) * distribB->index[n_rank]);
...
static void
_compute_local_numbering
(
 PDM_ol_t       *ol,
 PDM_ol_mesh_t   imesh,
 PDM_g_num_t     n_g_newVtx,
 PDM_g_num_t     nTSubFaces,
 int           **firstRecvStride,
 PDM_g_num_t   **firstRecv,
 int           **secondRecvStride,
 PDM_g_num_t   **secondRecv,
 int           **thirdRecvStride,
 double        **thirdRecv,
 int           **fourthRecvStride,
 PDM_g_num_t   **fourthRecv
 )
{
  PDM_surf_mesh_t *mesh    = NULL;
  if (imesh == PDM_OL_MESH_A) {
    mesh = ol->meshA;
  } else {
    mesh = ol->meshB;
  }
  const int n_part = PDM_surf_mesh_n_part_get (mesh);
  ...
  for (int i = 0; i < n_part; i++) {
    ...
    int idx = 0;
    for (int j = 0; j < n_face_part; j++) {
      if (firstRecvStride[i][j] == 0) {
        nUnChangedFacePart[i] += 1;
        s_olface_vtx[i] += iniface_vtx_idx[j+1] - iniface_vtx_idx[j];
      }
      else {
        nSubFacePart[i] += firstRecvStride[i][j]/5;
        for (int k = 0; k < firstRecvStride[i][j]/5; k++) {
          int n_vtx = (int) firstRecv[i][idx++];
          idx++;
          PDM_g_num_t oNumAbs = firstRecv[i][idx++];
          idx += 2;

          s_olface_vtx[i] += n_vtx;

          assert(oNumAbs == gNumFace[j]);
        }
      }
    }
    nUnChangedFace += nUnChangedFacePart[i];
    nSubFace += nSubFacePart[i];
  }

  PDM_g_num_t begUnChangedFace = 0;
  PDM_MPI_Scan (&nUnChangedFace, &begUnChangedFace,
                1, PDM__PDM_MPI_G_NUM, PDM_MPI_SUM, ol->comm);

  begUnChangedFace += -nUnChangedFace + 1;
  ...
  _ol_mesh_t *ol_mesh = _ol_mesh_create (nGFace, nGVtx, n_part);
  if (imesh == PDM_OL_MESH_A) {
    ol->olMeshA = ol_mesh;
  } else {
    ol->olMeshB = ol_mesh;
  ...
  PDM_g_num_t iniNGVtx = PDM_surf_mesh_n_g_vtx_get (mesh);

  for (int i = 0; i < n_part; i++) {

    _ol_part_t *olp = _ol_part_create (mesh->part[i]);
    ol_mesh->part[i] = olp;
    ...
    olp->nLinkedFace = nSubFacePart[i];

    olp->initToOlFaceIdx = malloc (sizeof(int) * (n_face + 1));
    olp->initToOlFaceIdx[0] = 0;

    olp->initToOlFace = malloc (sizeof(int) * n_facePart);
    olp->linkedFaces =  malloc (sizeof(int) * 4 * nSubFacePart[i]);

    int *face_vtx_idxPart = malloc (sizeof(int) * (n_facePart + 1));
    face_vtx_idxPart[0] = 0;

    PDM_g_num_t *gface_vtxPart = malloc (sizeof(PDM_g_num_t) * s_olface_vtx[i]);
    int *face_vtxPart = malloc(sizeof(int) * s_olface_vtx[i]);
    ...
    for (int j = 0; j < n_face; j++) {
      ...
      if (firstRecvStride[i][j] == 0) {
        initToOlTmpN[j] = 1;
        face_vtx_idxPart[n_facePartStored+1] = face_vtx_idxPart[n_facePartStored] +
          (iniface_vtx_idx[j+1] - iniface_vtx_idx[j]);
        for (int k = iniface_vtx_idx[j]; k < iniface_vtx_idx[j+1]; k++) {
          face_vtxPart[idx1++] = iniface_vtx[k];
        }
        initToOlTmp1[n_facePartStored] = n_facePartStored+1;
        face_ln_to_gn[n_facePartStored] = nTSubFaces
          + begUnChangedFace
          + idxIpart
          + 1;
        idxIpart += 1;
    ...
    const PDM_g_num_t *iniGNumVtx = PDM_surf_mesh_part_vtx_g_num_get (mesh, i);
    const int ini_n_vtx = PDM_surf_mesh_part_n_vtx_get (mesh, i);

    PDM_g_num_t *cpIniGNumVtx = malloc (sizeof(PDM_g_num_t) * ini_n_vtx);
    int *orderVtx = malloc (sizeof(int) * ini_n_vtx);
    for (int j = 0; j < ini_n_vtx; j++) {
      cpIniGNumVtx[j] = iniGNumVtx[j];
      orderVtx[j] = j+1;
    }

    PDM_sort_long (cpIniGNumVtx, orderVtx, ini_n_vtx);
    ...
    for (int j = 0; j < n_face; j++) {
      ...
      if (firstRecvStride[i][j] > 0) {
        initToOlTmpN[j] = firstRecvStride[i][j]/5;
        for (int k = 0; k < firstRecvStride[i][j]/5; k++) {
          int n_vtx = (int) firstRecv[i][idx++];
          PDM_g_num_t numAbs = firstRecv[i][idx++];
          idx++;
          int iProc = (int) firstRecv[i][idx++];
          int iPart = (int) firstRecv[i][idx++];
          ...
          if (iProc >= 0) {
            ...
            olp->linkedFaces[4*n_linked_faces+1] = iProc;
            olp->linkedFaces[4*n_linked_faces+2] = iPart;
            olp->linkedFaces[4*n_linked_faces+3] = -1; // Come from other mesh in the next step
            n_linked_faces++;
          }
          ...
          for (int k1 = 0; k1 < n_vtx; k1++) {
            PDM_g_num_t currentVtx = secondRecv[i][idx2++];
            if (currentVtx <= iniNGVtx) {
              int idxVtx = PDM_binary_search_long (currentVtx,
                                                   cpIniGNumVtx,
                                                   ini_n_vtx);
              assert (idxVtx != -1);
              face_vtxPart[idx1++] = orderVtx[idxVtx];
            }
            else {
    ...
    int inin_vtx = PDM_surf_mesh_part_n_vtx_get (mesh, i);
    for (int j = 0; j < s_olface_vtx[i]; j++) {
    ...
    for (int j = 0; j < inin_vtx; j++) {
      vtx_ln_to_gn_part[j] = iniGNumVtx[j];
    }
    ...
    const double *iniVtx = PDM_surf_mesh_part_vtx_get (mesh, i);
    ...
    for (int j = 0; j < n_face; j++) {
      if (firstRecvStride[i][j] > 0) {
        for (int k = 0; k < firstRecvStride[i][j]/5; k++) {
          PDM_g_num_t n_vtx    = firstRecv[i][idx++];
          idx += 4;
          ...
          for (int k4 = 0; k4 < n_vtx; k4++) {
            ...
            coordsPart[3*ordervtx_ln_to_gn_part[idxVtx]    ] = thirdRecv[i][idx3++];
            coordsPart[3*ordervtx_ln_to_gn_part[idxVtx] + 1] = thirdRecv[i][idx3++];
            coordsPart[3*ordervtx_ln_to_gn_part[idxVtx] + 2] = thirdRecv[i][idx3++];
          }
        }
      }
    }
    ...
    for (int j = 0; j < n_face; j++) {
      if (fourthRecvStride[i][j] > 0) {
        olp->faceIniVtxIdx[j+1] = olp->faceIniVtxIdx[j] + fourthRecvStride[i][j];
      }
      else {
    ...
    for (int j = 0; j < n_face; j++) {
      if (fourthRecvStride[i][j] > 0) {
        for (int k = 0; k < fourthRecvStride[i][j]; k++) {
          PDM_g_num_t currentVtx = fourthRecv[i][idx2++];
          int idxVtx = PDM_binary_search_long (currentVtx,
                                               cpvtx_ln_to_gn_part,
                                               n_vtxPart);
          assert(idxVtx != -1);
        ...
        for (int k = iniface_vtx_idx[j]; k < iniface_vtx_idx[j+1]; k++) {
          olp->faceIniVtx[idx++] = iniface_vtx[k];
  ...
  if (1 == 0) {
    ...
    printf("*** Maillage resultat %c : \n", mesh_name);
    for (int i = 0; i < n_part; i++) {
      printf("\n  ******  Partition : %d\n", i);
      PDM_surf_part_dump (ol_mesh->part[i]->part);
      int n_face = PDM_surf_mesh_part_n_face_get (mesh, i);
      printf(" *** faceIniVtx :\n");
      for (int j = 0; j < n_face; j++) {
        for (int k = ol_mesh->part[i]->faceIniVtxIdx[j]; k < ol_mesh->part[i]->faceIniVtxIdx[j+1]; k++) {
          printf(" %d", ol_mesh->part[i]->faceIniVtx[k]);
        }
        printf("\n");
      }
      printf(" ***  initToOlFaceIdx:\n");
      for (int j = 0; j < n_face; j++) {
        for (int k = ol_mesh->part[i]->initToOlFaceIdx[j]; k < ol_mesh->part[i]->initToOlFaceIdx[j+1]; k++) {
          printf(" %d", ol_mesh->part[i]->initToOlFace[k]);
        }
        printf("\n");
      }
  ...
  for (int i = 0; i < n_part; i++) {
    free (firstRecv[i]);
    free (firstRecvStride[i]);
    free (secondRecv[i]);
    free (secondRecvStride[i]);
    free (thirdRecv[i]);
    free (thirdRecvStride[i]);
    free (fourthRecv[i]);
    free (fourthRecvStride[i]);
  }
  free (firstRecv);
  free (firstRecvStride);
  free (secondRecv);
  free (secondRecvStride);
  free (thirdRecv);
  free (thirdRecvStride);
  free (fourthRecv);
  free (fourthRecvStride);

  free (nUnChangedFacePart);
  free (nSubFacePart);
  free (s_olface_vtx);
}
 ...
 )
{
  ...
  for (int imesh = 0; imesh < 2; imesh++) {
    ...
    for (int i = 0; i < n_part; i++) {
      ...
      for (int j = 0; j < partFaceEdgeIdx[nEltPart]; j++) {
        ...
        for (int k = 0; k < 3; k++) {
          _face_vtxCooOrigin[3*j+k] = partVtxCoord[3*vtx1+k];
          _face_vtxNormalOrigin[3*j+k] = vtx_normal[imesh][i][3*vtx1+k];
        }
      }
      free (vtx_normal[imesh][i]);
    } // End of loop on parts (i)
    free (vtx_normal[imesh]);
  ...
  if (1) {
    ...
    int n_tmpA = 100;
    int n_tmpB = 100;
    double *ptsA = malloc (sizeof(double) * n_tmpA * 3);
    double *ptsB = malloc (sizeof(double) * n_tmpB * 3);
    ...
    for (int i = 0; i < n_elt_blockA; i++) {
      ...
      for (int j = blockA_boxesB_idx[i]; j < blockA_boxesB_idx[i+1]; j++) {
        ...
        for (int ii = 0; ii < n_vtxA; ii++) {
          for (int jj = 0; jj < n_vtxB; jj++) {
            int nData = 0;
            PDM_edges_intersect_res_t **_eir =
              PDM_edges_intersect_get (intersect,
                                       PDM_EDGES_GET_FROM_AB,
                                       PDM_ABS(_faceToEdgeA[ii]),
                                       PDM_ABS(_faceToEdgeB[jj]),
                                       &nData);
            ...
            PDM_edges_intersect_res_data_get (*_eir,
                                              PDM_EDGES_INTERSECT_MESHA,
                                              &nGEdgeA,
                                              &originEdgeA,
                                              &endEdgeA,
            ...
            if (n_ptsA + nNewPointsA >= n_tmpA) {
              n_tmpA = PDM_MAX (2*n_tmpA, n_ptsA + nNewPointsA);
              ptsA = realloc (ptsA, sizeof(double) * n_tmpA * 3);
            }
            for (int iii = 0; iii < nNewPointsA; iii++) {
              //printf("[%d] A %d : u = %f\n", i_rank, n_ptsA, uA[iii]);
              for (int l = 0; l < 3; l++) {
                ptsA[3*n_ptsA+l] = coordsA[3*iii+l];
              }
              n_ptsA++;
            }

            if (n_ptsB + nNewPointsB >= n_tmpB) {
              n_tmpB = PDM_MAX (2*n_tmpB, n_ptsB + nNewPointsB);
              ptsB = realloc (ptsB, sizeof(double) * n_tmpB * 3);
            }
            for (int jjj = 0; jjj < nNewPointsB; jjj++) {
              //printf("[%d] B %d : u = %f\n", i_rank, n_ptsB, uB[jjj]);
              for (int l = 0; l < 3; l++) {
                ptsB[3*n_ptsB+l] = coordsB[3*jjj+l];
              }
              n_ptsB++;
            }
          }
    ...
    sprintf(filename, "edge_intersectionsA_%2.2d.vtk", i_rank);
    f = fopen(filename, "w");
    fprintf(f, "# vtk DataFile Version 2.0\neiA\nASCII\nDATASET UNSTRUCTURED_GRID\n");

    fprintf(f, "POINTS %d double\n", n_ptsA);
    for (int i = 0; i < n_ptsA; i++) {
      for (int j = 0; j < 3; j++) {
        fprintf(f, "%lf ", ptsA[3*i+j]);
      }
      fprintf(f, "\n");
    }
    ...
    sprintf(filename, "edge_intersectionsB_%2.2d.vtk", i_rank);
    f = fopen(filename, "w");
    fprintf(f, "# vtk DataFile Version 2.0\neiB\nASCII\nDATASET UNSTRUCTURED_GRID\n");

    fprintf(f, "POINTS %d double\n", n_ptsB);
    for (int i = 0; i < n_ptsB; i++) {
      for (int j = 0; j < 3; j++) {
        fprintf(f, "%lf ", ptsB[3*i+j]);
      }
      fprintf(f, "\n");
    }
