      ...
      if(op == PDM_MPI_SUM) {
        ...
        for(int i = 0; i < pcg->n_entity_graph[i_part]; ++i) {
          ...
          for (int i_stride = 0; i_stride < stride; i_stride++) {
            _pdata[i_part][stride*i_entity+i_stride] += _recv_data[i_part][stride*i+i_stride];
          }
        }
      } else if (op == PDM_MPI_MAX) {
        for(int i = 0; i < pcg->n_entity_graph[i_part]; ++i) {
          int i_entity = pcg->pentity_graph[i_part][4*i]-1;
          for (int i_stride = 0; i_stride < stride; i_stride++) {
            _pdata[i_part][stride*i_entity+i_stride] = PDM_MAX(_pdata[i_part][stride*i_entity+i_stride],_recv_data[i_part][stride*i+i_stride]);
          }
        }
      } else if (op == PDM_MPI_MIN) {
        for(int i = 0; i < pcg->n_entity_graph[i_part]; ++i) {
          int i_entity = pcg->pentity_graph[i_part][4*i]-1;
          for (int i_stride = 0; i_stride < stride; i_stride++) {
            _pdata[i_part][stride*i_entity+i_stride] = PDM_MIN(_pdata[i_part][stride*i_entity+i_stride],_recv_data[i_part][stride*i+i_stride]);
          }
        }
      }
    }
